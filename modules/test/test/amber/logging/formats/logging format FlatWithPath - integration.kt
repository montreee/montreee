...
class `logging format FlatWithPath - integration` : AnnotationSpec() {

    @Test
    fun `logging with format FlatWithPath - integration`() {
        val logs = mutableListOf<String>()
        val logger = Logger(testString()).apply {
            onLog {
                logs.add(Formats.FlatWithPath(this))
            }
        }
    ...
    @Test
    fun `logging with format FlatWithPath and parent logger - integration`() {
        val logs = mutableListOf<String>()
        ...
        logsToTest.forEach {
            ...
            logs.first() shouldBe "" +
                    ...
                    "\n${it.time.toTimeStamp()} ${it.level} [${parent1.name}|${parent2.name}]:" +
                    "\n${it.time.toTimeStamp()} ${it.level} [${parent1.name}|${parent2.name}|${parent3.name}]:" +
                    "\n${it.time.toTimeStamp()} ${it.level} " +
